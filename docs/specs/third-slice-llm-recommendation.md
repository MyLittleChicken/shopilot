...
- **ClaudeAdapter 실키 런타임 검증(의도적 후순위)**: 어댑터는 타입체크 + 청크 매핑(`toLLMChunks`) 단위 테스트까지만 검증됨. 실제 `ANTHROPIC_API_KEY`로 `messages.stream` 호출·모델 ID 수락·실 스트리밍 이터레이션은 **라이브 미검증**(잔여 리스크). 목/폴백 경로는 전부 검증됨. 키 확보 시 스모크 테스트로 닫는다.
